### タイトル: 「JekyllブログをDocusaurusに移行する」
**日付:** 2024-07-22 17:54:45 +0900  
**タグ:** [docusaurus, blog, algolia, react]  
**カテゴリ:** null  
**著者:** haril  
**説明:** 「JekyllブログをDocusaurusに移行する際に直面した問題とその解決策の概要。」
...
- 2024年4月、ブログが突然ダウンしました。
    ...
    - 時間が経てば解決するかもしれませんが、積み重なった不便さから、自分が書いたコードに問題があると誤解してしまいました。
- Jekyllは制約が多く、メンテナンスが難しかったため、Docusaurusに移行することにしました。
    - 特にブログがフォークで構築されていたため、GitHubの貢献が記録されず、がっかりしました。
- O2でDocusaurusのサポートを求める既存の問題がありましたが、私はそれを使ったことがなく、解決できませんでした - [\[FR\] support mkdocs-material, docusaurus · songkg7/o2 · Discussion #346 · GitHub](https://github.com/songkg7/o2/discussions/346)
...
TypeScript。最近のプロジェクトではTSから離れる試みもありますが、個人的にはその必要性を感じませんでした。JSよりもTSを好むので、環境をTSで設定しました。

## パッケージマネージャー

Docusaurusはnpm、yarn、pnpmをサポートしています。npmを多用してきたので、今回はpnpmかyarnを試してみたかったです。

最終的に、以下の理由でyarnを選びました：

- pnpmでGitHub Actionsを設定する際に気に入らない点がいくつかありました。
...
Docusaurusはドキュメントモードとブログモードの両方を提供します。開発ブログにはブログモードだけで十分なので、ドキュメントページを削除することを考えました。しかし、これではメインのランディングページがなくなり、デザイン要素が失われるように感じました。

考えた末、ランディングページを保持し、ブログ専用にするのではなく、ドキュメントの形式を変更することにしました。

## Mermaid

Mermaidはコードで簡単に図を作成できるツールで、Docusaurusはプラグインを通じてこれをサポートしています。追加しましょう。
...
Javaはデフォルトでサポートされていないため、prism設定を通じてbashと一緒に追加しました。頻繁に使用する言語がハイライトされていない場合は、必要に応じて追加できます。
...
デプロイ方法はいくつかありますが、すべてをGitHub内で処理したかったので、GitHub Pagesを選びました。デフォルトのドメイン（`~.github.io`）もシンプルで便利です。
...
デフォルトのGitHubドメイン（`~.github.io`）を使用するのも良いですが、ブログをよりプロフェッショナルに見せるためにドメインを購入することにしました。カスタムドメインを持つことで、将来的にプラットフォームを変更する際にもSEOを維持しやすくなります。
...
- [Orama](https://askorama.ai/)
    ...
    - OpenAI統合によるセマンティック検索をサポート。
    - AlgoliaよりもUIが美しいです。個人的には機能と同じくらいデザインを重視します。
...
しかし、Oramaはまだ[韓国語のトークン化](https://docs.askorama.ai/open-source/supported-languages/)をサポートしていないため、韓国語の検索ができず、ロールバックせざるを得ませんでした 😭。主な読者が韓国語を話すため、Algoliaに戻しました。
...
したがって、Algoliaインデックスを作成する際には、これらのファセットを**設定する必要があります**。多くの場合、DocSearchインデックスが作成される際にこれらのファセットが設定されていないことがあります。

これは、DocSearchの承認後にDocusaurus設定が反映される前にクローラーが実行されると発生することがあります。
...
- Algoliaインデックスを直接クエリすると、新しい記事が表示されました。
- contextualSearchをローカルで無効にしても効果がありませんでした。
- 問題はAPIにあるようでした。
- クローラーをトリガーした直後の検索結果は正しかったが、新しいウィンドウでテストすると古いバージョンに戻りました。
- 分散検索ネットワーク（DSN）技術がインデックスの更新反映に遅延を引き起こしている可能性がありました。
- 問題は約1日後に解決しました。

## i18n

英語圏からの訪問者が多く、英語のブログを運営して練習と広範なリーチを目指したいと思いました。Docusaurusはi18nをサポートしているので、これを実装することにしました。
...
- 韓国語で書かれた記事を英語に翻訳したい。
- `title.md`ファイルのみがPRに含まれている場合、それを翻訳して`/en/title.md`として含めることができるか？
- これに対するよく知られた解決策はないようです。
- Sweep AIを試しましたが、翻訳用に設計されていませんでした。
- GitHub Action + Open AIまたはDeepL APIを使用するのはどうでしょうか？
- 基本的なページ（404ページなど）はホスティングサービスによって翻訳ページにリダイレクトされない場合があります。GitHub Pagesはこれをサポートしていません。

このようなGitHubアクションが既に存在することがわかりました（偉大な頭脳は同じことを考えるものです）。
...
更新はgit履歴に基づいて判断されるため、GitHub Actionでクローンする際のdepthオプションを確認してください。デフォルトでは最新のコミットのみをフェッチします。
